...
class RecipeCard extends HTMLElement {
  ...
  constructor() {
    ...
    // A1. Attach the shadow DOM to this Web Component, in mode open
    const shadow = this.attachShadow({ mode: "open" });

    // A2. Create an <article> element - This will hold our markup once data is set
    const article = document.createElement('article');

    // A3. Create a style element - This will hold all of the styles for the Web Component
    const style = document.createElement('style');

    // A4. Insert all of the styles from cardTemplate.html into the <style> element created
    style.textContent = 
  `
  {
  ...
  set data(data) {
    ...
    if (!data) return;
    // A6. Select the <article> from Shadow DOM in the constructor
